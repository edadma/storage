...
//	println( pwidth, lowestSize, sizeShift, bucketLen )
  ...
  def getObject = {
    ...
    def chunk: Unit = {
      ...
      while (pos - start < len) {
        if (getUnsignedByte == USED) map += getValue -> getValue
        else {
          skipValue()
          skipValue()
        }
        ...
        new AbstractIterator[Long] {
          ...
          def next(): Long =
            if (hasNext) {
              scan = true
              cur
            } else throw new NoSuchElementException("next on empty objectIterator")
        }
    ...
    getType(addr) match {
      ...
      case ELEMENTS =>
        ...
        new AbstractIterator[Long] {
          ...
          def next(): Long =
            if (done) throw new NoSuchElementException("next on empty arrayIterator")
            else {
              val res = cur
      ...
      t match {
        ...
        case ELEMENTS =>
          def chunk: Unit = {
            ...
            while (pos - start < len) {
              if (getUnsignedByte == USED) checkvalue
              else {
                checkbytes(vwidth)
                skipValue()
              }
